    ...
    <div class="container-fluid py-5">
        <div class="container py-5">
            <div class="reservation-header text-center">
                ...
                <a href="{{ route('cars') }}" class="new-reservation-btn">
                                ...
                                <div class="row no-gutters">
                                    <div class="col-md-5">
                                        <div class="car-img-container">
                                            <img src="../assets/{{ $location->voiture->photo }}" class="car-img" alt="{{ $location->voiture->marque }}">
                                        </div>
                                    </div>
                                    <div class="col-md-7">
                                        <div class="card-body">
                                            <h5 class="car-title">{{ $location->voiture->marque }} {{ $location->voiture->modele }}</h5>

                                            <div class="reservation-detail">
                                                <i class="far fa-calendar-alt"></i>
                                                <strong>From:</strong> {{ $location->date_debut }}
                                            </div>
                                            <div class="reservation-detail">
                                                <i class="far fa-calendar-check"></i>
                                                <strong>To:</strong> {{ $location->date_fin }}
                                            </div>
                                            <div class="reservation-detail">
                                                <i class="fas fa-money-bill-wave"></i>
                                                <strong>Total:</strong> {{ $location->montant_total }} DH
                                            </div>
                                            <div class="reservation-detail">
                                                <i class="fas fa-file-contract"></i>
                                                <strong>Contract:</strong> {{ $location->contrat->typeContrat ?? 'N/A' }}
                                            </div>
                                            <div class="mt-2">
                                                @if($location->status == 'confirmé')
                                                    <span class="badge badge-success">{{ $location->status }}</span>
                                                @elseif($location->status == 'en attente')
                                                    <span class="badge badge-warning text-dark">{{ $location->status }}</span>
                                                @else
                                                    <span class="badge badge-secondary">{{ $location->status }}</span>
                                                @endif
                                            </div>
                                        </div>
                                    </div>
